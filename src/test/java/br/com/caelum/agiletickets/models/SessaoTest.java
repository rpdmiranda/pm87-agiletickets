...
public class SessaoTest {
	...
	@Test
	public void deveCriarApenasUmaSessaoQuandoAsDatasForemIguais()
			throws Exception {
		Espetaculo espetaculo = new Espetaculo();
		List<Sessao> sessoes = espetaculo.criaSessoes(
				LocalDate.parse("2018-07-05"), LocalDate.parse("2018-07-05"),
				LocalTime.parse("22:00"), Periodicidade.DIARIA);
	...
	public void deveCriarDoisEspetaculos() throws Exception {
		Espetaculo espetaculo = new Espetaculo();
		List<Sessao> sessoes = espetaculo.criaSessoes(
				LocalDate.parse("2018-07-05"), LocalDate.parse("2018-07-12"),
				LocalTime.parse("22:00"), Periodicidade.SEMANAL);

		Assert.assertNotNull(sessoes);
		Assert.assertEquals("Duas sessões", 2, sessoes.size());
		Assert.assertEquals(new LocalDate(2018, 7, 5), sessoes.get(0)
				.getInicio().toLocalDate());
		Assert.assertEquals(new LocalDate(2018, 7, 12), sessoes.get(1)
				.getInicio().toLocalDate());
	}
	...
	public void naoDeveCriarDataFimMaiorDataInicioSemanal() throws Exception {
		Espetaculo espetaculo = new Espetaculo();
		List<Sessao> sessoes = espetaculo.criaSessoes(
				LocalDate.parse("2018-07-12"), LocalDate.parse("2018-07-05"),
				LocalTime.parse("22:00"), Periodicidade.SEMANAL);

		Assert.assertNotNull(sessoes);
		Assert.assertEquals("Sem sessões criadas", 0, sessoes.size());
		// Assert.assertEquals(new LocalDate(2018,7,5),
		// sessoes.get(0).getInicio().toLocalDate());
		// Assert.assertEquals(new LocalDate(2018,7,12),
		// sessoes.get(1).getInicio().toLocalDate());
	}
	...
	public void naoDeveCriarSemDataInicioDiario() throws Exception {
		...
		Espetaculo espetaculo = new Espetaculo();
		try {
			espetaculo.criaSessoes(null, LocalDate.parse("2018-07-05"),
					LocalTime.parse("22:00"), Periodicidade.DIARIA);
			Assert.fail("Não deveria criar ssessões sem data inicial");
